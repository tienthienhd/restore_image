...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, fold=0, patch_size=128, patch_num_per_image=1, max_trdata=12000):
        ...
        if fold != 0:
            ...
            logging.info('Loading training images information...')
            self.imgfiles =sorted([join(imgs_dir + '/input', file) for file in listdir(imgs_dir + '/input')
    ...
    def __getitem__(self, i):
        ...
        gt_awb_file = img_file.replace('/input/', '/gt/')
        ...
        return {
            'image': torch.from_numpy(in_img_patches), 'gt-AWB': torch.from_numpy(awb_img_patches),
        }
